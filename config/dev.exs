...
config :rs, RsWeb.Endpoint,
  ...
  code_reloader: true,
  debug_errors: true,
  secret_key_base: "pkMZKnCpfz51wi/R5kDhS+9IInZcbgsKdi+AzzmtkHFW7te8hc0Tkir/MJv95Glx",
  watchers: [
    esbuild: {Esbuild, :install_and_run, [:rs, ~w(--sourcemap=inline --watch)]},
    tailwind: {Tailwind, :install_and_run, [:rs, ~w(--watch)]}
  ]
...
config :rs, RsWeb.Endpoint,
  live_reload: [
    ...
    patterns: [
      ...
      ~r"priv/gettext/.*(po)$",
      ~r"lib/rs_web/(?:controllers|live|components|router)/?.*\.(ex|heex)$"
    ]
  ]
